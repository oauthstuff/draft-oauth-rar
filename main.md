...
# Token Response
The authorization details assigned to the access token issued in a token response are determined by the `authorization_detail` and `resource` parameters of the corresponding token request.  If the client does not specify any of those token request parameters, the AS determines those authorization details at its discretion.
